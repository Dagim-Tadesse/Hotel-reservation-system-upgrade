...
const Bookings = () => {
  ...
  const handleAdd = async () => {
    if (!form.guest_id || !form.room_id || !form.check_in_date || !form.check_out_date) { toast.error("All fields required"); return; }
    ...
    setShowForm(false); setForm({ guest_id: "", room_id: "", check_in_date: "", check_out_date: "" }); loadData();
  };

  const handleDelete = async (booking: Booking) => {
    if (!confirm("Delete this booking?")) return;
    await supabase.from("bookings").delete().eq("id", booking.id);
    toast.success("Booking deleted"); loadData();
  };

  const togglePaid = async (booking: Booking) => {
    await supabase.from("bookings").update({ is_paid: !booking.is_paid }).eq("id", booking.id);
    toast.success(booking.is_paid ? "Marked as unpaid" : "Marked as paid"); loadData();
  };
